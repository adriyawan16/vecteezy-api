...
const regex = /https:\/\/www\.vecteezy\.com\/(video|psd|bundle|vector-art)\/(\d+)-/;
const MAX_FILENAME_LENGTH = 59;
function generateUrls(inputUrl) {
  ...
  if (match && match[1]) {
    ...
    let folderStructure = "";
    if (mediaType === "video" || mediaType === "psd" || mediaType === "bundle") {
      folderStructure = `${formattedVideoId.substr(0, 3)}/${formattedVideoId.substr(3, 3)}/${formattedVideoId.substr(6)}`;
    ...
    // Generate URLs for each file extension
    const generatedUrls = [];
    for (const extension of fileExtensions) {
      const newUrl = `https://files.vecteezy.com/system/protected/files/${folderStructure}/vecteezy_${fileName}_${videoId}.${extension}`;
